...
- `jobs._run_pipeline` に `track_std` 分岐を追加。track 既定値を `std` にする。
...
`label_zeroshot`・`label_vlm_single`・`label_gemini` は同一インターフェース（入力＝映像＋語彙＋境界、出力＝`SegmentList`）を保ち差し替え可能（元spec原則の踏襲）。
...
| `label_gemini` | JSONパース、category正規化、語彙外ラベル追記、時刻クランプ | Gemini をモック |
| 窓・stitch | **境界整列**（粗区間を割らない）、オーバーラップ採用ルール、**不変条件**（連続・非重複・全長被覆） | なし（純関数） |
| `propose_labels` | フレームサンプリング、提案パース、失敗時の空リスト | Gemini をモック |
| `parse_reference` | テキスト抽出、画像フォールバック分岐 | pdf/Gemini をモック |
